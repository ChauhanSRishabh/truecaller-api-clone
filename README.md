...
Also generates a unique Authorization Token which is shown to the user as it would be needed to be entered for further requests by the user to validate whether he is logged in or not(refer [instructions.txt](instructions.txt) to see how to pass this auth_token). 
...
Would do the same if we don’t pass any Authorization Token.